...
def plot_with_labels(low_dim_embs, labels, filename):   # 绘制词向量图
    ...
    mark = ['darkorange', 'greenyellow', 'lightskyblue', 'violet']
    color = []
    l = defaultdict(list)
...
def load_txt_vec(file, threshold=0, dtype='float'):	# 读取txt文件格式的word2vec词向量
    ...
    for i in range(count):
        ...
        words.append(i)   #l ine[0]
        vec = line
        matrix[i] = np.array(vec)  # np.fromstring(vec, sep=' ', dtype=dtype)
    print("#####")
    return (words, matrix)
...
if __name__ == '__main__':
    try:
        ...
        method = 'dblp_deepwalk.txt'
        w2v_txt_file = open('./dataset/'+method, 'r')	# txt格式的词向量文件
        ...
        np.save('fig_stne/' + method[:-4] + '.npy', low_dim_embs)
        labels=read_label("./dataset/DBLP/labels.txt")
        # low_dim_embs = np.load('node_ax.npy')
        #labels = [words[i] for i in range(plot_only)] # 要显示的点对应的单词列表
        print('paint picture')
        plot_with_labels(low_dim_embs, labels, 'fig_stne/' + method[:-4] + '.png')
